...
| `@nw/secrets` | Prompt Vault only required process-local `getSecret`/`storeSecret` fallback behavior. That narrow behavior is now app-local and refuses insecure production fallback unless explicitly overridden. Network deployments should inject `JWT_SECRET`. |
...
1. Consolidate logging behind the existing app logger.
2. Make event-bus, widget, and orchestrator registration optional integrations.
3. Extract shared auth/Core DB behavior behind an interface.
4. Extract `@nw/tags-projects` last because it affects prompt enrichment, project scoping, imports, exports, and tool behavior.
