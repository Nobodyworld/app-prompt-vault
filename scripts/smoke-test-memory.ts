...
(function () {
  ...
  if (useInMemory) {
    try {
      ...
      console.error('Failed to apply migrations to in-memory DB:', err && (err as Error).message ? (err as Error).message : err);
      process.exit(5);
    }
  }
  ...
  try {
    ...
    console.error('Smoke test error:', err && (err as Error).message ? (err as Error).message : err);
    process.exit(4);
  } finally {
    try {
      db.close();
