...
static SLUG_PATTERN: Lazy<Regex> = Lazy::new(|| Regex::new(r"^[a-z0-9]+(?:-[a-z0-9]+)*$").expect("invalid slug regex"));
...
#[tauri::command]
async fn create_prompt(state: State<'_, AppState>, payload: CreatePromptPayload) -> Result<CreatePromptResponse, String> {
...
fn create_prompt_inner(state: State<'_, AppState>, payload: CreatePromptPayload) -> Result<CreatePromptResponse, AppError> {
  ...
  .map_err(|error| match error {
    SqlError::SqliteFailure(ref sqlite_error, _) if sqlite_error.code == ErrorCode::ConstraintViolation => {
...
fn compose_prompt_summary(connection: &Connection, partial: &PartialPrompt) -> Result<PromptSummary, AppError> {
...
fn fetch_latest_version(connection: &Connection, prompt_id: &str) -> Result<Option<PromptVersionSummary>, AppError> {
  let mut stmt = connection.prepare(
    "SELECT id, semantic_version, updated_at
...
fn validate_payload(payload: &CreatePromptPayload) -> Result<(), AppError> {
  ...
  if !SLUG_PATTERN.is_match(payload.slug.trim()) {
    return Err(AppError::Validation("Slug must use lowercase letters, numbers, and hyphens".into()));
  }
  ...
  Version::parse(payload.semantic_version.trim())
    .map_err(|_| AppError::Validation("Semantic version must follow MAJOR.MINOR.PATCH".into()))?;
...
fn main() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![list_prompts, create_prompt])
