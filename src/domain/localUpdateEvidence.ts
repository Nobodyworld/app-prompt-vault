...
function summarizeMsi(msi: MsiObservation | null): {
  ...
  if (!msi) return null;
  const file = msi.media.flatMap((medium) => medium.members).find((member) => member.key === executableRow(msi)?.key)?.file;
  return {
...
function metadataValid(msi: MsiObservation): boolean {
  return msi.properties.ProductName === "Prompt Vault" && msi.properties.Manufacturer === "Nobody Production"
...
function executableRow(msi: MsiObservation): MsiObservation["files"][number] | null {
  const rows = msi.files.filter((file) => longName(file.name).toLowerCase() === "prompt-vault-app.exe");
...
function verifyManifest(msi: MsiObservation, manifest: LocalUpdateManifest, label: string, blockers: string[]): void {
  if (!metadataValid(msi)) blockers.push(`${label}:foreign-or-unsupported-msi`);
...
function verifyPayload(msi: MsiObservation, manifest: LocalUpdateManifest, label: string, blockers: string[]): boolean {
  const row = executableRow(msi);
  ...
  const valid = !!row && !!payload && manifest.executable.relativePath === "prompt-vault-app.exe"
...
function reconcile(observation: UpdateObservation, blockers: string[]): InstalledUpdateIdentity | null {
  const start = blockers.length;
  ...
  if (!metadataValid(cached) || record.displayName !== "Prompt Vault" || record.publisher !== "Nobody Production" || record.windowsInstaller !== "1") blockers.push("installed:foreign-target");
  const installScope = record.source.startsWith("HKLM/") ? "per-machine" : "per-user";
  const service = record.services[0]; const row = executableRow(cached);
  ...
  if (!location || path !== location + "\\prompt-vault-app.exe" || canonicalPath(service.installLocation) !== location || (/\.exe$/i.test(icon) && canonicalPath(icon) !== path)) blockers.push("installed:location-contradiction");
  ...
  return { applicationIdentifier: PROMPT_VAULT_IDENTIFIER, version: record.displayVersion, executableSha256: exe.sha256, productCode: normalizeMsiGuid(record.key)!, upgradeCode: normalizeMsiGuid(cached.properties.UpgradeCode)!, packageCode: normalizeMsiGuid(cached.packageCode)!, installScope };
}
...
export function planObservedLocalUpdate(manifestInput: unknown, observationInput: unknown, recoveryInput?: unknown): ReadOnlyUpdateReport {
  ...
  const checked = validateLocalUpdateManifest(manifestInput);
  ...
  let payload = false;
  if (selected) {
    if (!metadataValid(selected)) blockers.push("selected:foreign-or-unsupported-msi");
    const complete = verifyMedia(selected, extension(manifestInput, "media"), "selected", blockers);
    if (manifest) {
      verifyManifest(selected, manifest, "selected", blockers);
      payload = verifyPayload(selected, manifest, "selected", blockers) && complete;
    }
  }
  const installed = observation ? reconcile(observation, blockers) : null;
  ...
  return {
    ...
    selected: summarizeMsi(selected), selectedManifest: { valid: !!manifest, sourceCommit: manifest?.application.sourceCommit ?? null, artifactVerified: selectedValid },
    ...
    recovery: { ready: recoveryReady, procedureVerified, media: summarizeMsi(recovery) }, payloadCorrespondenceProven: payload,
