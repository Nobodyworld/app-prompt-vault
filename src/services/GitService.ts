...
export class GitService {
    ...
    async init(): Promise<void> {
        await this.git.init();
        await this.git.addConfig('user.name', this.config.authorName || 'Prompt Vault');
        await this.git.addConfig('user.email', this.config.authorEmail || 'prompt-vault@localhost');
     ...
     * Get commit history
     */
    async log(maxCount: number = 10): Promise<Array<{
        hash: string;
        date: string;
        message: string;
        author_name: string;
    ...
    async hasConflicts(): Promise<boolean> {
        ...
        return status.files.some(file => file.working_dir === 'U' || file.index === 'U');
    }
