...
import { createRateLimitMiddleware, InMemoryRateLimitStore } from "./rate-limit.js";
...
function createMockReq(ip = "127.0.0.1"): MockRequest {
    return { ip, socket: { remoteAddress: ip } as unknown as Request["socket"], locals: {} };
}

describe("rate-limit middleware", () => {
    it("allows requests within the window and sets headers", async () => {
        const store = new InMemoryRateLimitStore();
        const middleware = createRateLimitMiddleware({ maxRequests: 2, windowMs: 1000, store });
    ...
    it("returns 429 and calls onLimitExceeded when limit is exceeded", async () => {
        ...
        const middleware = createRateLimitMiddleware({ maxRequests: 1, windowMs: 10_000, store, onLimitExceeded });
